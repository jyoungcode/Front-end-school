...
// parameter에 0에서 ++로 +1이 되고 write()에서 +1이 적용된다.
// 그래서 16까지 출력이 된다.
while (count++ <= 15) {
...
####for 문
 ...
 - `for`문 순서 : 1) 조건문 2) 선언문 3) 실행코드 4) i++
...
for ( ; k < l; k++) {
	...
	if ( isType(fruit) !== 'string') {
		// 다음 for문으로 점프! string이 아니면 건너띄겠다.
		continue
	}
...
// 해당 부모에 자식들을 찾을때 ( 요소인 것만 )
var collection = document.body.childNodes,
	el_collection = []

for (var i = collection.length - 1 ; collection[i]; i-- ){
	// ES6 let : var와 다르게 지역변수로만 사용된다.
	// ES6는 크롬에서만 실행된다. (아직까지)
	// let은 hoisting 되지 않는다.
